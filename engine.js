...
class Engine {
    ...
    for (const seat of ["1", "2"]) {
      ...
      for (const lane of LANES) {
        for (const unit of this.state.players[seat][laneKey(lane)].slice()) {
          ...
          // (Wheelie-like cards opt back in via scope:"both_players" above.)
  ...
  fireBorrowedTrigger(unit, seat, lane, trigger, ctx) {
    ...
    for (const s of unit.statuses.filter(function(st) { return st.kind === "borrowed_passive_trigger"; })) {
      ...
      // - move ops with destination:"auto_flee" execute immediately (e.g. Mirette)
      // Calling runPassiveEffects blindly skips both, causing Maxine-copy to
      // auto-flee without a picker and counter-damage to miss the attacker.
