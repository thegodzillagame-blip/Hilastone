...
function freshGameState(hand1, hand2) {
  return {
    ...
    currentTurn: "1",
    turnNumber: 1,
    // "On the first turn, when the first player summons a card, the second
    // player gains 1 bonus Energy." This fires exactly once — for the very
    // first summon of the whole game — not on every game-opening turn.
    firstSummonBonusGiven: false,
    log: [],
  };
}
...
class Engine {
  ...
  summon(seat, cardName, lane, opts) {
    ...
    // "On the first turn, when the first player summons a card, the second
    // player gains 1 bonus Energy." Fires once total, on the very first
    // summon of the game, regardless of which seat makes it.
    if (!this.state.firstSummonBonusGiven) {
      this.state.firstSummonBonusGiven = true;
      const oppSeat = this.opponentOf(seat);
      const oppP = this.state.players[oppSeat];
      oppP.energy += 1;
      this.emit("energy_change", { seat: oppSeat, amount: 1, newTotal: oppP.energy, reason: "first_summon_bonus" });
    }
  ...
  getContinuousState(unit) {
    ...
    // immune_to_disable is a standalone always-on flag that can be tacked
    // onto a card whose PRIMARY passive trigger is something other than
    // passive_continuous (Astaroth's on_damaged reflect, Hyperion's
    // on_attacked bonus) — e.g. "Cannot be disabled. <other effect>." on
    // a single card. Scan for it separately so it isn't silently dropped
    // just because the main passive lives under a different trigger.
    if (selfDef.passive && selfDef.passive.trigger !== "passive_continuous") {
      for (const eff of selfDef.passive.effects) {
        if (eff.op === "immune_to_disable") bag.cannotBeDisabled = true;
      }
    }
  ...
  resolveTargetUnits(selector, frame, eff) {
    ...
    switch (selector) {
      case "literal_unit":
        // Used by ops that already hold a direct unit reference (e.g. Ellie
        // Ember's per-card lane coin flip) rather than resolving from a
        // player-chosen instance id.
        return targets && targets.literalUnit ? [targets.literalUnit] : [];
      case "self":
  ...
  op_damage(eff, frame) {
    ...
    // Rios a2: "If fatal to either, this action is free." Same refund as
    // onFatalRefund, but keyed off the SELF-damage being fatal instead of
    // (or in addition to) the main target. Guarded so we don't double-refund
    // if both happen to be fatal in the same resolution.
    if (eff.onSelfFatalRefund && selfFatal && !(eff.onFatalRefund && mainTargetFatal)) {
      const p = this.state.players[frame.actorSeat];
      p.energy += frame.spentCost || 0;
      this.emit("energy_change", { seat: frame.actorSeat, amount: frame.spentCost || 0, newTotal: p.energy, reason: "fatal_refund" });
    }
  ...
  // Ellie Ember: "Every turn, all cards in this lane flip a coin. If Tails,
  // they are disabled (stunned) for the turn." Each card in her lane —
  // friend or foe, including Ellie herself — gets an independent coin flip.
  // Tails applies a "this_turn" stun, which lapses naturally at her owner's
  // own next turn boundary via the same expiry machinery as every other
  // "this_turn" status (see durationToExpiry / expireStatusesOnTurnStart).
  // Immune units (cannotBeDisabled, e.g. Leon/Astaroth/Hyperion/Halcyon/
  // Orina/Jacie) are skipped by op_disable_actions itself, so no extra
  // guard is needed here.
  op_coin_flip_disable_lane(eff, frame) {
    if (!frame.actorLane) return;
    const units = [];
    for (const seat of ["1", "2"]) {
      units.push(...this.state.players[seat][laneKey(frame.actorLane)]);
    }
    for (const unit of units) {
      const heads = this.rng() < 0.5;
      this.emit("coin_flip", { heads: heads, actorInstanceId: unit.instanceId });
      if (!heads) {
        this.op_disable_actions(
          { op: "disable_actions", target: "literal_unit", duration: "this_turn" },
          { actorUnit: frame.actorUnit, actorSeat: frame.actorSeat, actorLane: frame.actorLane, targets: { literalUnit: unit } }
        );
      }
    }
  }
