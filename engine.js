...
const HAND_SIZE = 10;
const BASE_ENERGY = 5;
const TURN_CLOCK_MS = 10 * 60 * 1000; // 10 minutes per player, like a chess clock
  ...
  durationToExpiry(duration, ownerSeat) {
    switch (duration) {
      ...
      case "until_their_next_turn":
        // Distinct from "until_next_turn": this expires at the start of the
        // STATUS HOLDER's own next turn (e.g. Kazura's "disable their
        // actions for one turn" — the disabled enemy, not Kazura, is who
        // the clock is measured against). ownerSeat here is intentionally
        // the target's seat — see op_disable_actions, which passes the
        // resolved unit's own seat rather than frame.actorSeat for this
