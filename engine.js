...
const HAND_SIZE = 10;
const BASE_ENERGY = 5;
const TURN_CLOCK_MS = 5 * 60 * 1000; // 5 minutes per player, like a chess clock
...
class Engine {
  ...
  summon(seat, cardName, lane, opts) {
    ...
    this.firePassiveTrigger("on_place", { seat: seat, lane: lane, instanceId: unit.instanceId });
    ...
    for (const casterSeat of ["1", "2"]) {
      ...
      for (const pending of p.pendingEffects) {
        ...
        const runFrame = {
          ...
          actorLane: pending.frame.actorLane,
  ...
  firePassiveTrigger(trigger, ctx) {
    for (const seat of ["1", "2"]) {
      // on_turn_start (non-Wheelie) and on_turn_end_zero_energy are inherently
      // "this seat's own units react to their own turn boundary" triggers —
      // skip the other seat entirely so e.g. Shelby on seat 2 doesn't see
      // seat 1 ending their turn at 0 energy.
      if ((trigger === "on_turn_end_zero_energy") && seat !== ctx.seat) continue;
      for (const lane of LANES) {
  ...
  resolveAction(seat, instanceId, actionIndex, targets) {
    ...
    for (const eff of action.effects) {
      this.runOp(eff, { actorUnit: unit, actorSeat: seat, actorLane: lane, targets: targets, isPassive: false, spentCost: cost });
    }
  ...
  op_damage(eff, frame) {
    ...
    for (const unit of units) {
      ...
      const result = this.applyDamage(unit, amount, frame.actorUnit, frame.actorSeat, {
        multiTarget: units.length > 1 || eff.target.indexOf("all_") === 0,
  ...
  applyDamage(targetUnit, baseAmount, attackerUnit, attackerSeat, opts) {
    ...
    if (!isSelfDamage) {
      const defenderState = this.getContinuousState(unit);
  ...
  maybeMarauд(attackerUnit, defenderUnit) {
    ...
    if (othersInLane.length > 0) {
      this.runOp(
        ...
        { actorUnit: attackerUnit, actorSeat: attackerFound.seat, actorLane: defenderFoundAfter.lane, targets: {} }
      );
    }
  }
  ...
  maybeOnAttacked(defender, attacker, multiTarget) {
    ...
    for (const eff of def.passive.effects) {
      if (eff.op === "once_per_turn_gate") {
        ...
        this.runOp(eff, { actorUnit: defender, actorSeat: found.seat, actorLane: found.lane, targets: { destination: this.pickFleeLane(found.lane) } });
  ...
  op_heal(eff, frame) {
    ...
    for (const unit of units) {
      const found = findUnit(this.state, unit.instanceId);
      if (!found) continue;
      // hardCap: card-specific cap (e.g. Audrey "HP cannot exceed 4", Lilith
      // "HP cannot exceed 10") that's lower than the unit's max HP would
      // otherwise allow. Falls back to effective max HP if unset.
      const cap = eff.hardCap !== undefined ? eff.hardCap : this.getEffectiveMaxHp(found.unit);
      const before = found.unit.hp;
      found.unit.hp = Math.min(cap, found.unit.hp + amount);
  ...
  op_disable_actions(eff, frame) {
    const units = this.resolveTargetUnits(eff.target, frame, eff);
    for (const unit of units) {
      addStatus(unit, { kind: "disable_actions", expires: this.durationToExpiry(eff.duration, frame.actorSeat) });
  ...
  defeatUnit(instanceId, byUnit, opts) {
    ...
    this.cascadeLinkedDefeats(unit.name);
    return true;
  }
  ...
  cascadeLinkedDefeats(defeatedName) {
    for (const seat of ["1", "2"]) {
      for (const lane of LANES) {
        for (const unit of this.state.players[seat][laneKey(lane)].slice()) {
          const def = this.cardDef(unit.name);
  ...
  // own lane if not explicitly supplied (acceptable default for a
  // single-player-driven choice; a real UI would always supply both).
